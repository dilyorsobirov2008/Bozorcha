...
async def show_statistics_callback(callback: CallbackQuery, state: FSMContext, session: AsyncSession) -> None:
    ...
    try:
        ...
        today_sum = today.get("total_sum", 0) if isinstance(today, dict) else 0

        # Parse monthly stats
        monthly_count = monthly.get("count", 0) if isinstance(monthly, dict) else 0
        monthly_sum = monthly.get("total_sum", 0) if isinstance(monthly, dict) else 0

        # Parse total stats
        total_count = total.get("count", 0) if isinstance(total, dict) else 0
        total_sum = total.get("total_sum", 0) if isinstance(total, dict) else 0

        # Parse best sellers
        best_lines = []
        if best:
            for i, item in enumerate(best[:10], 1):
                name = item.get("product_name", "—") if isinstance(item, dict) else getattr(item, "product_name", "—")
                qty = item.get("total_qty", 0) if isinstance(item, dict) else getattr(item, "total_qty", 0)
