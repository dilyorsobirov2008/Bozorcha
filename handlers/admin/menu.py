...
async def admin_categories(callback: CallbackQuery, state: FSMContext, session: AsyncSession) -> None:
    ...
    try:
        ...
        logger.info("Categories loaded: count=%d", len(categories))
        if not categories:
...
async def admin_products(callback: CallbackQuery, state: FSMContext, session: AsyncSession) -> None:
    ...
    try:
        ...
        categories = await get_categories(session)
        logger.info("Products menu: categories loaded, count=%d", len(categories))
...
async def admin_orders(callback: CallbackQuery, state: FSMContext, session: AsyncSession) -> None:
    ...
    try:
        ...
        logger.info("Orders loaded: count=%d", len(orders))
        if not orders:
...
# ── 📖 Instruktsiya Handler ──────────────────────────────────────────────
@router.callback_query(F.data == "admin_instruction")
async def admin_instruction(callback: CallbackQuery, state: FSMContext) -> None:
    logger.info("🎯 Triggered callback: admin_instruction")
    if not await _is_admin(callback, state):
        await callback.answer("⛔ Ruxsat yo'q", show_alert=True)
        return
    try:
        ...
        instruction_text = (
            ...
            "📁 <b>Kategoriya qo'shish:</b>\n"
            "1. Categories ni bosing\n"
            "2. Add Category ni bosing\n"
            "3. Nom kiriting\n"
            "4. Saqlanadi\n\n"
            "📦 <b>Mahsulot qo'shish:</b>\n"
            "1. Products ni bosing\n"
            "2. Add Product ni bosing\n"
            "3. Nom kiriting\n"
            "4. Narx kiriting\n"
            "5. Rasm yuboring\n"
            "6. Kategoriyani tanlang\n"
            "7. Saqlanadi\n\n"
            "✏️ <b>Tahrirlash:</b>\n"
            "Elementni tanlab Edit bosing\n\n"
            "🗑 <b>O'chirish:</b>\n"
            "Elementni tanlab Delete bosing"
        )
        await callback.message.edit_text(
            instruction_text,
        ...
        logger.error("admin_instruction error: %s", exc, exc_info=True)
